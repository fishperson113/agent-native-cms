...
import { GET as getArtifact } from "@/app/api/articles/[articleId]/presentation/route";
...
const otherPublishedId = "20000000-0000-4000-8000-000000000063";
...
beforeEach(async () => {
  ...
  await database.db.insert(tenants).values([
    { id: tenant, name: "M6 Tenant", slug: "m6-tenant" },
    { id: otherTenant, name: "Other publisher", slug: "other-publisher" },
  ]);
  await database.db.insert(articles).values([
    ...
    { id: otherPublishedId, tenantId: otherTenant, title: "Another public story", slug: "published-m6-story", markdown: "# Also public", status: "published" },
  ]);
...
describe("public delivery", () => {
  ...
  it("aggregates published articles across tenants and keeps drafts private", async () => {
    const result = await listPublicArticles();
    expect(result.map((article) => article.id).sort()).toEqual(
      [publishedId, otherPublishedId].sort(),
    );
    expect(result.map((article) => article.slug)).toEqual([
      "published-m6-story",
      "published-m6-story",
    ]);
    await expect(getPublicArticle(draftId)).rejects.toThrow();
    await expect(getPublicArticle(otherPublishedId)).resolves.toMatchObject({
  ...
  it("serves the active compiled artifact as versioned JavaScript", async () => {
    const response = await getArtifact(
      new Request(`http://localhost/api/articles/${publishedId}/presentation?version=${presentationId}`),
      { params: Promise.resolve({ articleId: publishedId }) },
    );
    expect(response.status).toBe(200);
